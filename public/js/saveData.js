...
var saveConsent = function(){
  db.collection('tasks').doc('av-ext').collection(version).doc(uid).set({
...
var saveStartData = function(startTime){
  db.collection('tasks').doc('av-ext').collection(version).doc(uid).update({
    taskStartTimePhaser: startTime,
    expCompleted: 0
  });
  // initialize data-storage collections
  db.collection('tasks').doc('av-ext').collection(version).doc(uid).collection('task-data').doc('data').set({init: 1});
  db.collection('tasks').doc('av-ext').collection(version).doc(uid).collection('post-task-data').doc('data').set({init: 1});
};

// function to save the main task data
var saveTaskData = function(trialN, dataToSave) {
  db.collection('tasks').doc('av-ext').collection(version).doc(uid).collection('task-data').doc('data').update({[trialN]: dataToSave});
};

// function to save the post-task questions data
var savePostTaskData = function(questN, dataToSave) {
  db.collection('tasks').doc('av-ext').collection(version).doc(uid).collection('post-task-data').doc('data').update({[questN]: dataToSave});
};
...
var saveTaskEndData = function(dataBackup){
  ...
  // db.collection('tasks').doc('av-ext').collection(version).doc(uid).collection('task-data').doc('data-backup').update(dataBackup);
};
...
var saveEndData = function(dataBackup){
  ...
  // db.collection('tasks').doc('av-ext').collection(version).doc(uid).collection('quest-data').doc('data-backup').update(dataBackup);
};

export { saveConsent, saveStartData, saveTaskData, savePostTaskData, saveTaskEndData, saveEndData }
